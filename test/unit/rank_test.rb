  ...
  context "adding new ranks" do
    should "only allow ranks in the range 0-4" do
      ...
      allowed.each do |good|
        rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => good, :rank_start => @date)
        assert rank.valid?
      end

      not_allowed.each do |bad|
        rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => bad, :rank_start => @date)
        assert !rank.valid?
      end
    end

    should "allow multiple ranks for one person" do
      @staff_rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => 1, :rank_start => @date)
      assert @staff_rank.save
      assert_equal Rank.all.count, 2

      @coeditor_rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => 2, :rank_start => @date)
      assert @coeditor_rank.save
      assert_equal Rank.all.count, 3
      @coeditor_rank.destroy
      assert_equal Rank.all.count, 2

      @editor_rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => 3, :rank_start => @date)
      assert @editor_rank.save
      assert_equal Rank.all.count, 3
    end

    should "end person's current editorship if new one is made" do
      assert_equal Rank.all.count, 1

      @coeditor_rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => 2, :rank_start => @date)
      assert @coeditor_rank.save
      assert_equal Rank.all.count, 2
      
      @editor_rank = Rank.new(:person_id => @web_rank.person_id, :rank_type => 3, :rank_start => (@date >> 12))
      assert @editor_rank.save
      persons_editorships = Rank.find(:all, :conditions => "(rank_type=2 or rank_type=3) and rank_end IS NULL and person_id=#{@web_rank.person_id}")
